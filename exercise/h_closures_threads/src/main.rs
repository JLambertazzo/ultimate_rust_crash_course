...
fn main() {
    ...
    // the child threads.
    let (tc, rc): (crossbeam::Sender<&str>, crossbeam::Receiver<&str>) = channel::unbounded();
    let rc2 = rc.clone();

    let child1 = thread::spawn(|| {
        for msg in rc {
            println!("Child 1 receives {}", msg);
        }
    });

    let child2 = thread::spawn(|| {
        for msg in rc2 {
            println!("Child 2 receives {}", msg);
        }
    });

    for i in 1..=10 {
        tc.send("Hi there").unwrap();
    }
    drop(tc);
